...
fun order(a:Int){
    when(a) {
        1 -> {println("결제를 진행하겠습니다. 금액을 넣어 주십시요.")
            val putpay = pay(readLine()!!.toInt())}
        2 -> {println("메뉴판으로 돌아갑니다.")
            showMenu()
        }
    }
}
...
fun pay(putpay: Int) {
    val totalPrice = shoppingCart.sumBy { it.price }
    val count = putpay - totalPrice

    if (count >= 0) {
        println("결제가 완료되었습니다. 거스름돈은 $count 원입니다.")
        shoppingCart.clear() // 결제 완료 후 장바구니 비우기
    } else {
        println("금액이 부족합니다. 결제 할 수 없습니다.")
        order(1) // 결제를 다시 시도
    }
}